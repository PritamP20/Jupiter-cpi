...
describe("cpi-swap-program", () => {
  ...
  it("prepare accounts", async () => {
    ...
    if (balance < 0.1 * LAMPORTS_PER_SOL) {
      console.log("⚠️  Low SOL balance detected. You may need to fund your wallet for mainnet testing.");
    ...
    if (instructions.length > 0) {
      console.log(`💰 Creating ${instructions.length} token account(s) - this will cost ~${instructions.length * 0.00204} SOL`);
  ...
  it("swap SOL -> USDC through Jupiter", async () => {
    try {
      ...
      const { swapInstruction, setupInstructions, cleanupInstruction } = swapResponse as {
        swapInstruction: any;
    ...
    console.log("USDC balance after swap:", senderUsdcAcc.value.uiAmountString);
